var recoURL='https://192.168.4.147:5000/register';
var toURL='https://192.168.6.201:5000/face';
...
function imageExit(mes) {
    ...
    if(count<10){
        ...
        if(!isNaN(parseInt(remoteFaceid)))
        {
            alert("您已经注册过！！请登录")
...
function sendFaceId(uidValue, face_id) {

    // window.location.href = 'http://192.168.5.118:3000/user/login';

    let url = `http://192.168.5.108:3000/user/login/faceRegister?id=${uidValue}&face_id=${face_id}`
    let img = new Image();
    if(img.complete){
        alert("注册成功")
        window.location.href="http://192.168.5.108:3000/user/login"
        ...
        img.onload = function (event) {
            ...
            alert("注册成功")
            window.location.href="http://192.168.6.201:5000/scan"
...
function startScan() {
    ...
    setTimeout(function () {
	capTimer = setInterval(takePictureBefore, 1000);
